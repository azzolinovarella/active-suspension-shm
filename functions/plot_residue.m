function [fig1, fig2] = plot_residue(t, y_luenberger, y_hat_obsu_luenberger, ...
    ...
    y_max = round(max(max([r1_obsu_luenberger, r1_obs1_luenberger, r1_obsu_kalman, r1_obs1_kalman]))/5, 1, 'significant')*5;
    ...
    % xlim([t(1), t(end)])
    % Para corrigir os eixos e ficar mais bonito (multiplos de 5E?)
    y_max = round(max(max([r2_obsu_luenberger, r2_obs2_luenberger, r2_obsu_kalman, r2_obs2_kalman]))/5, 1, 'significant')*5;
    yticks(linspace(-2*y_max, 2*y_max, 11))
    xticks(t(1):5:t(end))
