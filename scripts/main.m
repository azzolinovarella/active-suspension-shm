...
[y_hlt_bode, ~, wout] = bode(sys_hlt, logspace(0, log10(k2*f0*2*pi), 1000));
...
for i = ['1']  % Cada caso da simulacao
    ...
    [y_dmg_bode, ~, wout] = bode(sys_dmg, logspace(0, log10(k2*f0*2*pi), 1000));
    ...
    figure
    hold on
    semilogx(wout, y1_h1t_bode)
    semilogx(wout, y1_dmg_bode)
    grid on
    xlabel('\omega (rad/s)')
    ...
    figure
    hold on
    semilogx(wout, y2_h1t_bode)
    semilogx(wout, y2_dmg_bode)
    grid on
    xlabel('\omega (rad/s)')
