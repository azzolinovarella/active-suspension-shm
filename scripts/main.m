...
%%% Filtro passa-baixas
[B_filter, A_filter] = butter(2, 70/(f_sr*pi), 'low');   % wc = 25 + wn2
...
% Salvando as figuras
%%% No tempo
plot_chirp(t, w, [t(1), t(end), -As*1.1, As*1.1], true, '../figs/chirp.png', false);
%%% Na frequencia
ii = ceil(length(t)/2);
w_ = w(1:ii);  % Fazendo assim pois dois concatenados da bug...
plot_fspec(w_, length(w_)*10, dt, [k1*f0, k2*f0+2, -20, 1], [k1*f0, k2*f0+2, -185, 185], true, true, '../figs/chirp_mag.png', false);
...
% Salvando as figuras
%%% Ruido 1
plot_noise(t, v(1,:), [0, 32, 1.1*min(v(1,:)), 1.1*max(v(1,:))], '1', 'm', true, '../figs/noise_v1.png', false);
plot_noise_dist(v(1,:), 30, '1', 'm', true, '../figs/noise_v1_dist.png', false);
%%% Ruido 2
plot_noise(t, v(2,:), [0, 32, 1.1*min(v(2,:)), 1.1*max(v(2,:))], '2', 'm/s', true, '../figs/noise_v2.png', false);
plot_noise_dist(v(2,:), 30, '2', 'm/s', true, '../figs/noise_v2_dist.png', false);
...
for i = ['1', '2', '3']  % Cada caso da simulacao
    ...
    plot_sys_bode(sys_hlt, sys_dmg, sim_case, k1*f0+eps, k2*f0 + 5, [0, 60, -35, 0], [0, 60, 15, 50], true, sprintf('../figs/bode_y1_c%s.png', sim_case), sprintf('../figs/bode_y2_c%s.png', sim_case), false);
    ...
    y1 = y_luenberger.y1;
    y1_hat = y_hat_luenberger.y1_hat;
    r1 = y1 - y1_hat;
    r1_filtered = filter(B_filter, A_filter, r1);
    figure('visible', 'on')
    subplot(2, 1, 1)
    plot(t_luenberger, r1)
    subplot(2, 1, 2)
    plot(t_luenberger, r1_filtered)
    
    y2 = y_luenberger.y2;
    y2_hat = y_hat_luenberger.y2_hat;
    r2 = y2 - y2_hat;
    r2_filtered = filter(B_filter, A_filter, r2);
    figure('visible', 'on')
    subplot(2, 1, 1)
    plot(t_luenberger, r2)
    subplot(2, 1, 2)
    plot(t_luenberger, r2_filtered)
    % figure('visible', 'off')
    % title(sprintf(''))
    %%% 
